...
class DropDownTree {
    ...
    private HandleDocumentMouseUp(evt: any) {
        const el = this._el;
    ...
    private HandleDocumentScroll(evt: any) {
        const el = this._el;
        let elTarget: HTMLElement = evt.target as HTMLElement;
        while ((elTarget != null) && (elTarget != el))
            elTarget = elTarget.parentElement;
    ...
    private async GetModelData(): Promise<string | string[]>{
        if (this._multiselection) {
            const selections: string[] | string = await this._app.Storage.RetrieveDataValue(this._sector, this._model);
