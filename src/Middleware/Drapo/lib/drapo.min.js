...
class DrapoFunctionHandler {
    ...
    ExecuteFunctionRemoveDataItem(sector, contextItem, element, event, functionParsed, executionContext) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const item = yield this.Application.Solver.ResolveItemDataPathObject(sector, contextItem, itemPath);
            if (item == null)
                return (null);
            const notifyText = functionParsed.Parameters[2];
            const notify = ((notifyText == null) || (notifyText == '')) ? true : yield this.Application.Solver.ResolveConditional(notifyText);
            const deleted = yield this.Application.Storage.DeleteDataItem(dataKey, mustacheParts, sector, item, notify);
            if (!deleted)
                return (null);
        });
    }
    ...
    ExecuteFunctionContainsDataItem(sector, contextItem, element, event, functionParsed, executionContext) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const item = yield this.Application.Solver.ResolveItemDataPathObject(sector, contextItem, dataItemPath);
...
class DrapoParser {
    ...
    ParseExpression(expression) {
        const normalizedExpression = this.ExpressionNormalizer(expression);
    ...
    ExpressionNormalizer(expression) {
        const expressionNormalized = expression.split('');
        let blockCount = 0;
        let textBlock = null;
        let buffer = '';
