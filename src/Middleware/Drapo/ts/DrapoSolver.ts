class DrapoSolver {
    ...
    public async ResolveConditional(expression: string | boolean | number, el: HTMLElement = null, sector: string = null, context: DrapoContext = null, renderContext: DrapoRenderContext = null, eljForTemplate: HTMLElement = null, executionContext: DrapoExecutionContext<any> = null): Promise<boolean> {
        ...
        //Resolve
        const response: string = await this.ResolveConditionalExpressionBlock(sector, context, renderContext, executionContext, el, eljForTemplate, block);
        //Recursive
    ...
    private async ResolveConditionalExpressionBlock(sector: string, context: DrapoContext, renderContext: DrapoRenderContext, executionContext: DrapoExecutionContext<any>, el: HTMLElement, eljForTemplate: HTMLElement, block: DrapoExpressionItem): Promise<string> {
        //Resolve Mustaches
        await this.EnsureExpressionItemCurrentLevelResolved(sector, context, renderContext, executionContext, el, block, eljForTemplate);
        //Join Texts
        this.JoinTexts(block);
        //Operation
        return (await this.ResolveConditionalExpressionBlockOperation(sector, context, renderContext, executionContext, el, eljForTemplate, block));
    }

    private async ResolveConditionalExpressionBlockOperation(sector: string, context: DrapoContext, renderContext: DrapoRenderContext, executionContext: DrapoExecutionContext<any>, el: HTMLElement, eljForTemplate: HTMLElement, block: DrapoExpressionItem): Promise<string> {
        if (block.Items.length === 0)
            return ('');
        //First
        await this.EnsureExpressionItemResolved(sector, context, renderContext, executionContext, el, block, 0, eljForTemplate);
        ...
        if ((itemFirst.Type == DrapoExpressionItemType.Logical) || (itemFirst.Type == DrapoExpressionItemType.Comparator)) {
            ...
            return (await this.ResolveConditionalExpressionBlock(sector, context, renderContext, executionContext, el, eljForTemplate, block));
        }
        const resultFirst: string = itemFirst.Value;
        if (block.Items.length < 2)
            return (resultFirst);
        //Second
        await this.EnsureExpressionItemResolved(sector, context, renderContext, executionContext, el, block, 1, eljForTemplate);
        ...
        if (hasMoreThanTwoTerms) {
            await this.EnsureExpressionItemResolved(sector, context, renderContext, executionContext, el, block, 2, eljForTemplate);
        ...
        if (resultThird === '!') {
            ...
            if (block.Items.length > 3) {
                await this.EnsureExpressionItemResolved(sector, context, renderContext, executionContext, el, block, 3, eljForTemplate);
    ...
    private async EnsureExpressionItemCurrentLevelResolved(sector: string, context: DrapoContext, renderContext: DrapoRenderContext, executionContext: DrapoExecutionContext<any>, el: HTMLElement, block: DrapoExpressionItem, eljForTemplate: HTMLElement): Promise<void> {
        for (let i: number = 0; i < block.Items.length; i++) {
            ...
            if (item.Type === DrapoExpressionItemType.Function)
                block.Items[i] = new DrapoExpressionItem(DrapoExpressionItemType.Text, (await this.Application.FunctionHandler.ReplaceFunctionExpressionsContext(sector, context, item.Value, true, executionContext)));
            else if (item.Type === DrapoExpressionItemType.Mustache)
                block.Items[i] = new DrapoExpressionItem(DrapoExpressionItemType.Text, (await this.Application.Barber.ResolveControlFlowMustacheString(context, renderContext, executionContext, item.Value, el, sector, true, DrapoStorageLinkType.Render, eljForTemplate != null, eljForTemplate)));
        }
    }
    ...
    private async EnsureExpressionItemResolved(sector: string, context: DrapoContext, renderContext: DrapoRenderContext, executionContext: DrapoExecutionContext<any>, el: HTMLElement, block: DrapoExpressionItem, index: number, eljForTemplate: HTMLElement): Promise<void> {
        const item: DrapoExpressionItem = block.Items[index];
        if (item.Type === DrapoExpressionItemType.Block)
            block.Items[index] = new DrapoExpressionItem(DrapoExpressionItemType.Text, (await this.ResolveConditionalExpressionBlock(sector, context, renderContext, executionContext, el, eljForTemplate, item)).toString());
        else if (item.Type === DrapoExpressionItemType.Function)
            block.Items[index] = new DrapoExpressionItem(DrapoExpressionItemType.Text, (await this.Application.FunctionHandler.ReplaceFunctionExpressionsContext(sector, context, item.Value, true, executionContext)));
        else if (item.Type === DrapoExpressionItemType.Mustache)
            block.Items[index] = new DrapoExpressionItem(DrapoExpressionItemType.Text, (await this.Application.Barber.ResolveControlFlowMustacheString(context, renderContext, executionContext, item.Value, el, sector, true, DrapoStorageLinkType.Render, eljForTemplate != null, eljForTemplate)));
    }
    ...
    public async ResolveDataPathMustache(context: DrapoContext, executionContext: DrapoExecutionContext<any>, element: HTMLElement, sector: string, mustacheParts: string[]): Promise<string> {
        let updated: boolean = false;
        for (let i: number = 1; i < mustacheParts.length; i++) {
            ...
            const dataValue = await this.ResolveDataPath(context, executionContext, element, sector, mustachePartParts);
    ...
    public async ResolveDataPath(context: DrapoContext, executionContext: DrapoExecutionContext<any>, element: HTMLElement, sector: string, path: (string[] | string), canBindReader: boolean = false, canBindWriter: boolean = false, modelEvents: string[] = null, modelEventsCancel: string[] = null, canNotify: boolean = true): Promise<string> {
        //Path
        ...
        for (let i: number = 1; i < dataPath.length; i++) {
            ...
            const mustacheValue = await this.ResolveDataPath(context, executionContext, element, sector, mustacheParts, canBindReader, canBindWriter, modelEvents, modelEventsCancel, canNotify);
        ...
        //Bind
        if (canBindWriter)
            this.Application.Binder.BindReaderWriter(await this.ResolveDataPathObjectItem(context.Item, dataKey, sector), element, dataFields, modelEvents, modelEventsCancel, canNotify);
        else if (canBindReader)
            this.Application.Binder.BindReader(await this.ResolveDataPathObjectItem(context.Item, dataKey, sector), element, dataFields);
        return (data);
    }
